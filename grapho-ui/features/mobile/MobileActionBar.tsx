...
export function MobileActionBar({ sidebarOpen, onToggleSidebar, onNewDocument, onFocusEditor, onExportPdf, onOpenStyle, disabled = false }: MobileActionBarProps) {
  ...
  const primary = (label: string, icon: React.ReactNode, onClick: () => void, active = false, actionDisabled = false) => <motion.button type="button" whileTap={{ scale: 0.94 }} onClick={onClick} disabled={actionDisabled} aria-label={label} aria-pressed={active}>{active && <motion.span layoutId="mobile-action-active" className="grapho-mobile-action-active" transition={{ type: "spring", stiffness: 420, damping: 30 }} />}{icon}</motion.button>;
  return <nav className={`grapho-mobile-action-bar grapho-floating-toolbar ${expanded ? "is-expanded" : ""}`} aria-label="Mobile workspace navigation">
    <div className="grapho-mobile-tab-capsule">
      ...
      {primary("PDF", <FileDown size={18} />, onExportPdf, false, disabled)}
      {primary(expanded ? "Close" : "More", <MoreHorizontal size={18} />, () => setExpanded((value) => !value), expanded)}
    </div>
    <motion.button type="button" whileTap={{ scale: .92 }} onClick={onNewDocument} disabled={disabled} aria-label="New document" className="grapho-mobile-primary-action"><Plus size={22} /></motion.button>
