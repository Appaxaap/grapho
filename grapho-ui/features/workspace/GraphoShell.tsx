...
export default function GraphoShell() {
  ...
  useEffect(() => {
    const handleDocumentNavigation = (event: KeyboardEvent) => {
      ...
      if (event.key === "F2") {
        event.preventDefault();
        renameDocument(focusedDocumentId ?? selectedId);
        return;
      }
      if (event.key === "Delete") {
        event.preventDefault();
        ...
        <AnimatePresence initial={false}>
          {sidebarOpen && <motion.aside initial={{ width: 0, opacity: 0 }} animate={{ width: 280, opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="grapho-sidebar hidden h-[calc(100vh-2rem)] shrink-0 overflow-hidden rounded-2xl border border-[var(--grapho-border)] bg-[var(--grapho-panel)] shadow-xl backdrop-blur-xl lg:sticky lg:top-4 lg:my-4 lg:ml-4 lg:block">
            <div className="flex h-full w-[280px] flex-col overflow-y-auto p-3 [scrollbar-width:none]">
              ...
              <div className="grapho-project-list mt-2 space-y-1">{folders.map((folder) => <button key={folder} type="button" onClick={() => { saveNow(); setActiveFolder(folder); setFocusedDocumentId(null); }} className={`flex h-9 w-full items-center gap-2 rounded-lg px-2.5 text-left text-[10px] transition-colors ${activeFolder === folder ? "bg-[var(--grapho-control)] text-[var(--grapho-foreground)]" : "text-[var(--grapho-muted)] hover:bg-[var(--grapho-control)]"}`}>{activeFolder === folder ? <FolderOpen size={14} /> : <Folder size={14} />}<span className="flex-1">{folder}</span><span className="text-[8px] text-[var(--grapho-faint)]">{documents.filter((item) => item.folder === folder).length}</span></button>)}</div>
