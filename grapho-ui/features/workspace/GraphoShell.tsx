    ...
    <div className={`grapho-ui ${theme === "dark" ? "grapho-dark" : ""} ${isNativeWindow ? "is-native-window" : ""} ${modalOpen ? "grapho-modal-open" : ""} relative min-h-screen overflow-hidden`}>
      ...
      <div className="grapho-workspace-layer relative z-10 flex min-h-screen">
        ...
        <main className="grapho-editor-scroll min-w-0 flex-1" aria-label="Writing canvas">
          <div style={{ fontFamily: editorFont === "Mono" ? "var(--grapho-font-mono)" : editorFont === "Serif" ? "Georgia, serif" : "var(--grapho-font-sans)", fontSize: editorSize === "Large" ? "18px" : "16px" }} className={`mx-auto ${editorWidth === "Wide" ? "max-w-6xl" : "max-w-4xl"} px-5 pb-32 pt-10 sm:px-12 sm:pt-14 lg:px-20 lg:pt-16 ${editorSpacing === "Compact" ? "[--grapho-leading-body:1.55]" : "[--grapho-leading-body:1.9]"}`}>
      ...
      <AnimatePresence>
        ...
        {renameTarget && <motion.div className="grapho-modal-backdrop fixed inset-0 z-[120] grid place-items-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) setRenameTarget(null); }}>
          <motion.section role="dialog" aria-modal="true" aria-labelledby="rename-document-title" initial={{ opacity: 0, y: 12, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="grapho-dialog grapho-glass w-full max-w-md rounded-3xl p-6">
            ...
            <input autoFocus value={renameDraft} onChange={(event) => setRenameDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") commitRename(); if (event.key === "Escape") setRenameTarget(null); }} aria-label="Document name" className="mt-6 h-11 w-full rounded-xl border border-[var(--grapho-border)] bg-[var(--grapho-control)] px-3 text-sm text-[var(--grapho-foreground)] outline-none focus:border-[var(--grapho-border)]" />
        ...
        {helpOpen && <motion.div className="fixed inset-0 z-[100] grid place-items-center bg-black/20 p-4 backdrop-blur-md" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) setHelpOpen(false); }}>
          <motion.section role="dialog" aria-modal="true" aria-labelledby="grapho-help-title" initial={{ opacity: 0, y: 18, scale: .96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: .97 }} transition={{ type: "spring", stiffness: 360, damping: 28 }} className="grapho-dialog grapho-glass w-full max-w-xl overflow-hidden rounded-3xl">
