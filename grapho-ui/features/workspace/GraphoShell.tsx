  ...
  return (
    <div style={{ "--grapho-accent": accentStyle === "Muted Ink" ? "#7B8792" : undefined, "--grapho-accent-soft": accentStyle === "Muted Ink" ? "rgba(123,135,146,.16)" : undefined } as CSSProperties} className={`grapho-ui ${theme === "dark" ? "grapho-dark" : ""} ${pageSurface === "Soft gray" ? "grapho-soft-surface" : ""} grapho-no-grid ${isNativeWindow ? "is-native-window" : ""} ${modalOpen ? "grapho-modal-open" : ""} relative min-h-screen overflow-hidden`}>
      ...
      <AnimatePresence>
        ...
        {blockDeleteConfirmOpen && <motion.div className="grapho-modal-backdrop fixed inset-0 z-[140] grid place-items-center bg-black/35 p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} onMouseDown={(event) => { if (event.target === event.currentTarget) setBlockDeleteConfirmOpen(false); }}><motion.section role="dialog" aria-modal="true" aria-labelledby="delete-blocks-title" className="grapho-dialog grapho-glass w-full max-w-sm overflow-hidden rounded-2xl p-2" initial={{ opacity: 0, y: 12, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }}><div className="px-3 py-3"><div className="text-[9px] uppercase tracking-[.18em] text-[var(--grapho-faint)]">Document blocks</div><h2 id="delete-blocks-title" className="mt-2 text-base font-semibold tracking-[-.03em]">Delete selected blocks?</h2><p className="mt-2 text-[10px] leading-5 text-[var(--grapho-muted)]">{selectedBlockIds.size} selected block{selectedBlockIds.size === 1 ? "" : "s"} will be removed from this document.</p></div><div className="flex justify-end gap-2 border-t border-[var(--grapho-border)] bg-[var(--grapho-control)] px-3 py-3"><button type="button" onClick={() => setBlockDeleteConfirmOpen(false)} className="rounded-xl px-4 py-2.5 text-[11px] text-[var(--grapho-muted)] hover:bg-[var(--grapho-control-hover)]">Cancel</button><button type="button" onClick={() => { removeSelectedBlocks(); setBlockDeleteConfirmOpen(false); }} className="rounded-xl bg-red-500 px-4 py-2.5 text-[11px] text-white">Delete blocks</button></div></motion.section></motion.div>}
