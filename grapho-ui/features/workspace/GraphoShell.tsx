      ...
      <div className="grapho-workspace-layer relative z-10 flex min-h-screen">
        ...
        <main className="grapho-editor-scroll min-w-0 flex-1" aria-label="Writing canvas">
          <div style={{ ...(editorFont === "Mono" ? { fontFamily: "var(--grapho-font-mono)" } : editorFont === "Serif" ? { fontFamily: "Georgia, serif" } : {}), ...(editorSize === "Large" ? { fontSize: "18px" } : {}) }} className={`mx-auto ${editorWidth === "Wide" ? "max-w-6xl" : "max-w-4xl"} px-5 pb-32 pt-10 sm:px-12 sm:pt-14 lg:px-20 lg:pt-16 ${editorSpacing === "Compact" ? "[--grapho-leading-body:1.55]" : "[--grapho-leading-body:1.9]"}`}>
            ...
            {intelligenceOpen && <DocumentIntelligencePanel result={intelligence} onShowProposed={() => { setAccountFilter("proposed"); setAccountView("register"); setIntelligenceOpen(false); }} onDuplicateCheck={() => setToast("Duplicate check: no duplicate account addresses found")} onCopyEmails={() => { const emails = [...selected.blocks].flatMap((block) => block.text.match(/[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}/g) ?? []); navigator.clipboard?.writeText([...new Set(emails)].join("\n")); setToast(`${new Set(emails).size} email addresses copied`); }} />}
...
function DocumentIntelligencePanel({ result, onShowProposed, onCopyEmails, onDuplicateCheck }: { result: ReturnType<typeof analyzeDocument>; onShowProposed: () => void; onCopyEmails: () => void; onDuplicateCheck: () => void }) {
