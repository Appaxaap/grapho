...
import { clearGraphoStorage, defaultGraphoPreferences, getGraphoStorageDiagnostics, loadGraphoStorage, normalizeEditorFont, saveGraphoStorage, type EditorFont, type GraphoPreferences } from "../../persistence/storage";
...
import { formatMemorandumOfUnderstanding, isMemorandumOfUnderstanding } from "../../domain/memorandum";
...
export default function GraphoShell() {
  ...
  const [editorFont, setEditorFont] = useState<EditorFont>(defaultGraphoPreferences.editorFont);
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      ...
      if (source) {
        ...
        setEditorFont(normalizeEditorFont(preferences.editorFont));
  ...
  const exportPdf = async () => {
    ...
    try {
      if (isNativeWindow) {
        const saved = await exportNativePdf(selected, { font: editorFont });
        setBusyAction(null);
  ...
  const pasteBlocks = (blockId: string, rawText: string) => {
    const isMou = isMemorandumOfUnderstanding(rawText);
    const formatted = isMou ? formatMemorandumOfUnderstanding(rawText, () => `pasted-${blockSequence.current++}`) : null;
    const parsed = formatted?.blocks ?? parseMarkdownBlocks(rawText, () => `pasted-${blockSequence.current++}`);
    if (!parsed.length) return;
    setDocuments((current) => current.map((document) => {
      if (document.id !== selected.id) return document;
      const index = document.blocks.findIndex((block) => block.id === blockId);
      const title = formatted?.title && /^untitled document$/i.test(document.title.trim()) ? formatted.title : document.title;
      return { ...document, title, updated: "Just now", blocks: [...document.blocks.slice(0, index), ...parsed, ...document.blocks.slice(index + 1)] };
    }));
    setCommandBlockId(null);
    setToast(isMou ? "Detected MoU · formatted as a model" : "Pasted content formatted");
  };
  ...
  const modalOpen = Boolean(deleteTarget || bulkDeleteTargets || renameTarget || paletteOpen || helpOpen || workspaceDialogOpen || workspaceRenameTarget || blockDeleteConfirmOpen);
    ...
    <div style={{ "--grapho-accent": accentStyle === "Muted Ink" ? "#7B8792" : undefined, "--grapho-accent-soft": accentStyle === "Muted Ink" ? "rgba(123,135,146,.16)" : undefined } as CSSProperties} className={`grapho-ui ${theme === "dark" ? "grapho-dark" : ""} ${pageSurface === "Soft gray" ? "grapho-soft-surface" : ""} ${focusMode ? "grapho-focus-mode" : ""} grapho-no-grid ${isNativeWindow ? "is-native-window" : ""} ${modalOpen ? "grapho-modal-open" : ""} relative min-h-screen overflow-hidden`}>
      ...
      <div className="grapho-workspace-layer relative z-10 flex min-h-screen">
        ...
        <main className="grapho-editor-scroll min-w-0 flex-1" aria-label="Writing canvas" onMouseDown={(event) => { const handle = (event.target as HTMLElement).closest<HTMLButtonElement>('button[aria-label="Drag or select block"]'); if (!handle) return; const editor = handle.closest("div.group")?.querySelector<HTMLElement>("[data-grapho-block-id]"); if (!editor?.dataset.graphoBlockId) return; event.preventDefault(); blockSelectionDragging.current = true; blockSelectionAnchor.current = editor.dataset.graphoBlockId; selectBlockRange(editor.dataset.graphoBlockId, event.shiftKey); }} onMouseOver={(event) => { if (!blockSelectionDragging.current) return; const editor = (event.target as HTMLElement).closest<HTMLElement>("[data-grapho-block-id]"); if (editor?.dataset.graphoBlockId) selectBlockRange(editor.dataset.graphoBlockId); }} onKeyDown={(event) => { if (event.key !== "Escape") return; if (selectedBlockIds.size) { event.preventDefault(); setSelectedBlockIds(new Set()); paintBlockSelection(new Set()); } }}>
          <div style={{ fontFamily: editorFont === "Inter" ? "var(--grapho-font-sans)" : editorFont === "Source Serif 4" ? "var(--grapho-font-serif)" : "var(--grapho-font)", ...(editorSize === "Large" ? { fontSize: "18px" } : {}) }} className={`mx-auto ${editorWidth === "Wide" ? "max-w-6xl" : "max-w-4xl"} px-5 pb-32 pt-10 sm:px-12 sm:pt-14 lg:px-20 lg:pt-16 ${editorSpacing === "Compact" ? "[--grapho-leading-body:1.55]" : "[--grapho-leading-body:1.9]"}`}>
            ...
            <article className="relative min-h-[620px]" onDragOver={(event) => event.preventDefault()} onDrop={handleMarkdownDrop} onMouseUp={handleCanvasSelection} onClick={(event) => { if (event.target === event.currentTarget) { const last = event.currentTarget.querySelector<HTMLElement>("[data-grapho-block]:last-of-type"); last?.focus(); } }}>
              <div className="mb-10"><EditableDocumentTitle value={selected.title} onChange={updateTitle} /></div>
      ...
      <AnimatePresence>
        ...
        {blockDeleteConfirmOpen && <motion.div className="grapho-modal-backdrop fixed inset-0 z-[140] grid place-items-center bg-black/35 p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} onMouseDown={(event) => { if (!deletingBlocks && event.target === event.currentTarget) setBlockDeleteConfirmOpen(false); }}><motion.section role="dialog" aria-modal="true" aria-labelledby="delete-blocks-title" className="grapho-dialog grapho-block-delete-dialog w-full max-w-md overflow-hidden rounded-2xl border border-[var(--grapho-border)] p-0 shadow-2xl" initial={{ opacity: 0, y: 12, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }}><header className="border-b border-[var(--grapho-border)] px-5 py-4"><div className="text-[9px] uppercase tracking-[.18em] text-[var(--grapho-faint)]">Document blocks</div><h2 id="delete-blocks-title" className="mt-2 text-lg font-semibold tracking-[-.04em] text-[var(--grapho-foreground)]">Delete selected blocks?</h2></header><div className="px-5 py-5"><p className="text-[11px] leading-5 text-[var(--grapho-muted)]">{selectedBlockIds.size} selected block{selectedBlockIds.size === 1 ? "" : "s"} will be removed from this document.</p></div><footer className="flex justify-end gap-2 border-t border-[var(--grapho-border)] bg-[var(--grapho-control)] px-5 py-3"><button type="button" onClick={() => setBlockDeleteConfirmOpen(false)} disabled={deletingBlocks} className="rounded-xl px-4 py-2.5 text-[11px] text-[var(--grapho-muted)] hover:bg-[var(--grapho-control-hover)] disabled:cursor-not-allowed disabled:opacity-40">Cancel</button><button type="button" onClick={() => { if (deletingBlocks) return; setDeletingBlocks(true); window.setTimeout(() => { removeSelectedBlocks(); setDeletingBlocks(false); setBlockDeleteConfirmOpen(false); }, 320); }} disabled={deletingBlocks} aria-busy={deletingBlocks} className="flex min-w-[130px] items-center justify-center gap-2 rounded-xl bg-red-500 px-4 py-2.5 text-[11px] text-white disabled:cursor-wait disabled:opacity-75"><span className={deletingBlocks ? "size-3.5 animate-spin rounded-full border-2 border-white/35 border-t-white" : "hidden"} aria-hidden="true" />{deletingBlocks ? "Deleting…" : "Delete blocks"}</button></footer></motion.section></motion.div>}
...
function MarkdownTableBlock({ text, onChange }: { text: string; onChange: (text: string) => void }) {
  ...
  const updateCell = (rowIndex: number, cellIndex: number, value: string) => {
    const nextRows = paddedRows.map((row) => [...row]);
    nextRows[rowIndex][cellIndex] = value;
    onChange(nextRows.map((row, index) => `| ${row.join(" | ")} |${index === 0 ? `\n| ${row.map(() => "---").join(" | ")} |` : ""}`).join("\n"));
  };
  ...
  return <div className="grapho-markdown-table my-4 overflow-x-auto rounded-2xl border border-[var(--grapho-border)] bg-[var(--grapho-control)]"><div className="grapho-table-heading"><div><span className="grapho-table-kicker">Structured table</span><strong>{paddedRows.length - 1} rows · {columns} columns</strong></div><span className="grapho-table-hint">Click any cell to edit</span></div><table className="w-full min-w-full table-fixed border-collapse text-left text-[11px] leading-5"><colgroup>{widths.map((width, index) => <col key={index} style={{ width: `${width}%` }} />)}</colgroup><thead><tr>{paddedRows[0].map((cell, index) => <th key={`${cell}-${index}`} className="relative border-b border-r border-[var(--grapho-border)] px-4 py-3 align-top font-semibold text-[var(--grapho-foreground)] break-words last:border-r-0"><input value={cell} onChange={(event) => updateCell(0, index, event.target.value)} aria-label={`Column ${index + 1} heading`} placeholder={`Column ${index + 1}`} className="w-full min-w-0 bg-transparent font-semibold text-[var(--grapho-foreground)] outline-none placeholder:text-[var(--grapho-faint)]" />{index < columns - 1 && <button type="button" aria-label={`Resize column ${index + 1}`} title="Drag to resize column" onPointerDown={(event) => resizeColumn(index, event)} className="grapho-table-resize-handle absolute -right-1 top-0 z-10 h-full w-2 cursor-col-resize touch-none" />}</th>)}</tr></thead><tbody>{paddedRows.slice(1).map((row, rowIndex) => <tr key={rowIndex} className="border-b border-[var(--grapho-border)] last:border-0">{row.map((cell, cellIndex) => <td key={`${cell}-${cellIndex}`} className="break-words border-r border-[var(--grapho-border)] px-4 py-3 align-top last:border-r-0"><input value={cell} onChange={(event) => updateCell(rowIndex + 1, cellIndex, event.target.value)} aria-label={`Row ${rowIndex + 1}, column ${cellIndex + 1}`} placeholder="Empty cell" className="w-full min-w-0 bg-transparent text-[var(--grapho-muted)] outline-none placeholder:text-[var(--grapho-faint)]" /></td>)}</tr>)}</tbody></table></div>;
}
...
function FontSelect({ value, onChange }: { value: EditorFont; onChange: (value: EditorFont) => void }) {
  return <label className="grapho-style-option mt-2 flex w-full items-center justify-between rounded-lg border border-transparent px-2.5 py-2.5 text-left text-[9px] hover:bg-[var(--grapho-control)]"><span><span className="grapho-style-option-label block text-[var(--grapho-muted)]">Document font</span><span className="mt-1 block text-[8px] text-[var(--grapho-faint)]">Choose the writing voice</span></span><span className="grapho-style-option-value flex items-center gap-2 text-[var(--grapho-foreground)]"><select value={value} onChange={(event) => onChange(event.target.value as EditorFont)} aria-label="Document font" className="grapho-font-select max-w-[148px] cursor-pointer appearance-none bg-transparent text-right outline-none"><option value="Grapho Geist Mono">Grapho Geist Mono</option><option value="Inter">Inter</option><option value="Source Serif 4">Source Serif 4</option></select><span className="grapho-style-option-arrow" aria-hidden="true">⌄</span></span></label>;
}
