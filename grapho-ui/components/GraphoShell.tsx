...
export default function GraphoShell() {
  ...
  return (
    <div className={`grapho-ui ${theme === "dark" ? "grapho-dark" : ""} ${isNativeWindow ? "is-native-window" : ""} relative min-h-screen overflow-hidden`}>
      ...
      <motion.div initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} transition={{ type: "spring", stiffness: 360, damping: 28 }} className="fixed right-4 top-4 z-50 flex items-center gap-1 rounded-2xl border border-[var(--grapho-border)] bg-[var(--grapho-panel)] p-1.5 shadow-xl">
      ...
      <div className="relative z-10 flex min-h-screen">
        <AnimatePresence initial={false}>
          {sidebarOpen && <motion.aside initial={{ width: 0, opacity: 0 }} animate={{ width: 280, opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="hidden h-[calc(100vh-2rem)] shrink-0 overflow-hidden rounded-2xl border border-[var(--grapho-border)] bg-[var(--grapho-panel)] shadow-xl lg:sticky lg:top-4 lg:my-4 lg:ml-4 lg:block">
      ...
      <AnimatePresence>
        ...
        {helpOpen && <motion.div className="fixed inset-0 z-[100] grid place-items-center bg-black/20 p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) setHelpOpen(false); }}>
